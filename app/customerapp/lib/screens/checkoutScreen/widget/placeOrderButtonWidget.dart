...
class _SwipeButtonState extends State<PlaceOrderButtonWidget> {
  ...
  void _handleRazorPayPaymentError(PaymentFailureResponse response) {
    ...
    GeneralMethods.showMessage(
        context, message["description"], MessageType.warning);
    setState(() {
  ...
  Future openPaystackPaymentGateway() async {
    ...
    if (response.status) {
      ...
      // GeneralMethods.showMessage(
      //     context, response.message, MessageType.warning);
    }
  }
      ...
      child: Widgets.gradientBtnWidget(
        ...
        callback: () async {
          if (widget.isEnabled ||
            ...
            if (context.read<CheckoutProvider>().selectedPaymentMethod ==
                "COD") {
        ...
        otherWidgets: (isPaymentUnderProcessing &&
                context.read<CheckoutProvider>().selectedAddress?.id != null)
            ? Container(
